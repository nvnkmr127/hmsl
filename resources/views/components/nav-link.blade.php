...
<a href="{{ $href }}"
   class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-bold transition-all duration-200 mb-1 group
    ...
    <span class="w-5 h-5 flex-shrink-0 transition-transform group-hover:scale-110">
        @switch($icon)
            ...
            @case('flask')
                ...
                @break
            @case('pill')
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                @break
            @case('box')
    ...
    <span class="uppercase tracking-widest text-[10px]">{{ $slot }}</span>
</a>
