<div>
    <x-page-header
        title="{{ $viewRecycleBin ? 'Recycle Bin: Deleted Patients' : 'Patient Registry' }}"
        subtitle="{{ $viewRecycleBin ? 'View and restore deleted patient records.' : 'View and manage all registered patients in the system.' }}">
        <x-slot name="actions">
            ...
            @if($viewRecycleBin)
                <button wire:click="$set('viewRecycleBin', false)" class="btn btn-secondary bg-gray-100 hover:bg-gray-200 text-gray-800 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-200 border-0 shadow-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                <button wire:click="$set('viewRecycleBin', true)" class="btn btn-secondary bg-amber-50/50 hover:bg-amber-100/50 text-amber-700 dark:bg-amber-950/20 dark:hover:bg-amber-900/30 dark:text-amber-400 border border-amber-200/50 dark:border-amber-900/30 shadow-sm relative">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                    </svg>
                    Recycle Bin
                    @if($stats['trashed'] > 0)
                        <span class="absolute -top-1.5 -right-1.5 px-2 py-0.5 text-[10px] font-black bg-rose-500 text-white rounded-full scale-90 border border-white dark:border-gray-950">{{ $stats['trashed'] }}</span>
                    @endif
                </button>
    ...
    @else
    <div class="p-5 mb-8 rounded-ultra bg-amber-500/10 border border-amber-500/20 text-amber-700 dark:text-amber-400 flex items-center gap-3">
        ...
        <div class="text-xs font-semibold uppercase tracking-wider">
            You are viewing the Recycle Bin. Restoring a patient record will bring back their full medical history, appointments, and billing data.
        </div>
    </div>
    @endif
                    ...
                    @if($viewRecycleBin)
                        <div class="flex items-start justify-between gap-4">
                            <div class="min-w-0 flex-1">
                                ...
                                <span class="text-base font-black text-gray-900 dark:text-white truncate block tracking-tight uppercase">{{ $patient->full_name }}</span>
                                ...
                                <div class="flex flex-col gap-1 mt-2">
                                    ...
                                    <div class="flex items-center gap-2 mt-1">
                                        ...
                                        <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Deleted: {{ $patient->deleted_at?->format('d M, h:i A') ?? 'N/A' }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-col gap-2 justify-center">
                                <button wire:click="restorePatient({{ $patient->id }})" 
                                        wire:confirm="Are you sure you want to restore this patient record?"
                                        class="p-3 bg-emerald-600 text-white rounded-2xl shadow-lg shadow-emerald-500/20 active:scale-95 transition-all"
                    ...
                    @else
                        ...
                        <div class="mt-4 pt-4 border-t border-gray-50 dark:border-gray-800 flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <button wire:click="deletePatient({{ $patient->id }})" wire:confirm="Are you sure you want to move this patient to the Recycle Bin?" class="px-3 py-2 text-[10px] font-black text-rose-400 uppercase tracking-widest hover:bg-rose-50 dark:hover:bg-rose-950/20 rounded-lg transition-colors">Delete</button>
                            </div>
                        </div>
                    @endif
                </div>
                            ...
                            <td class="py-5 pl-8">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="flex flex-col">
                                        @if($viewRecycleBin)
                                            <span class="font-black text-gray-900 dark:text-white uppercase tracking-tight">
                                                {{ $patient->full_name }}
                                            </span>
                                        @else
                            ...
                            @if($viewRecycleBin)
                                <td>
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <span class="font-black text-gray-700 dark:text-gray-300 text-xs tracking-tight">{{ $patient->deleted_at?->format('d M, Y h:i A') ?? 'N/A' }}</span>
                                    </div>
                                </td>
                            @else
                                <td>
                            ...
                            <td class="pr-8 text-right">
                                @if($viewRecycleBin)
                                    <div class="flex items-center justify-end gap-1">
                                        <button wire:click="restorePatient({{ $patient->id }})"
                                                wire:confirm="Are you sure you want to restore this patient record?"
                                                class="btn btn-ghost px-3 py-2 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/20 rounded-xl text-xs font-black uppercase tracking-wider inline-flex items-center gap-1.5" title="Restore Patient">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" />
                                            </svg>
                                            Restore
                                    ...
                                    <div class="flex items-center justify-end gap-1">
                                        ...
                                        <div class="flex items-center gap-0.5 ml-2 border-l border-gray-100 dark:border-gray-800 pl-2">
                                            ...
                                            <button wire:click="deletePatient({{ $patient->id }})"
                                                    wire:confirm="Are you sure you want to move this patient record to the Recycle Bin?"
                                                    class="btn btn-ghost px-2.5 py-2.5 text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/20 transition-all" title="Move to Recycle Bin">
