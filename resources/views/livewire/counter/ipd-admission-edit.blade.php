<x-modal name="edit-admission-modal" title="Edit Admission" width="4xl">
    @if($admission)
    <form wire:submit.prevent="save" class="space-y-8 p-6">
        <div class="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <x-clinical.patient-strip :patient="$admission->patient" size="lg" :active="true" />
        </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-stretch rounded-2xl bg-gray-50 dark:bg-gray-950 border-2 border-transparent focus-within:border-violet-500 transition-all shadow-sm ring-4 ring-gray-100/50 dark:ring-gray-900/50">
                            <input 
                                ...
                                class="flex-1 bg-transparent border-none focus:ring-0 pl-6 pr-6 py-5 font-black text-gray-900 dark:text-white text-sm outline-none uppercase tracking-wide"
                            >
                        </div>
                ...
                <div class="p-8 rounded-[2.5rem] bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-2xl shadow-indigo-500/5 space-y-8">
                    ...
                    @else
                        <div class="mt-6 py-6 bg-emerald-50 dark:bg-emerald-950 rounded-2xl border border-emerald-100 dark:border-emerald-800 text-center">
                            <p class="text-emerald-600 dark:text-emerald-400 font-bold">Internal bed will be auto-assigned for {{ $selectedWardObj->name ?? 'Ward' }}</p>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="pt-10 border-t border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-6">
            ...
            <div class="flex items-center gap-4 w-full sm:w-auto">
                <a href="{{ route('counter.ipd.show', $admission->id) }}" class="flex-1 sm:flex-none px-10 py-5 text-sm font-black uppercase tracking-widest text-gray-400 hover:text-gray-900 transition-colors text-center">Cancel</a>
                <button type="submit" class="flex-1 sm:flex-none px-12 py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-[1.5rem] text-sm font-black uppercase tracking-[0.2em] shadow-2xl shadow-indigo-500/40 transition-all active:scale-95 flex items-center justify-center gap-3">
                    <span wire:loading.remove>Update Admission</span>
