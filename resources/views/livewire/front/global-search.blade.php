<div class="relative flex-1 max-w-sm lg:max-w-md" 
     x-data="{ isOpen: @entangle('isOpen') }" 
     @click.away="isOpen = false"
     @open-global-search.window="$nextTick(() => $el.querySelector('input').focus())">
                    ...
                    <div class="p-2">
                        ...
                        @foreach($results['doctors'] as $doctor)
                            <a href="{{ route('master.doctors.index') }}?search={{ $doctor->full_name }}" class="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-all group">
                                ...
                                <div>
                                    <p class="text-sm font-bold text-gray-900 dark:text-white group-hover:text-primary-600">{{ $doctor->full_name }}</p>
