<div>
    ...
    <!-- Pending Bill Modal -->
    <x-modal name="pending-bill-modal" title="Pending Bill Settlement">
        <div class="p-6">
            <div class="mb-6 bg-amber-50 dark:bg-amber-900/30 p-4 rounded-xl border border-amber-200 dark:border-amber-800">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <h4 class="text-amber-800 dark:text-amber-400 font-bold">Unsettled Final Bill</h4>
                        <p class="text-sm text-amber-700 dark:text-amber-300 mt-1">
                            The patient has a pending balance of <span class="font-bold">₹{{ number_format($admission->finalBill?->balance_amount ?? 0, 2) }}</span>. 
                            Do you want to collect the payment now or discharge and collect later?
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3">
                <button type="button" @click="$dispatch('close-modal', { name: 'pending-bill-modal' })" class="btn btn-ghost">Cancel</button>
                <button type="button" @click="$dispatch('close-modal', { name: 'pending-bill-modal' })" class="btn border border-gray-300 text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700">Collect Later</button>
                <a href="{{ route('billing.index', ['collect_bill' => $admission->finalBill?->id]) }}" class="btn btn-primary">Collect Payment</a>
            </div>
        </div>
    </x-modal>
</div>
